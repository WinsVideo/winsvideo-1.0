...
class Account {
    ...
    public function insertUserDetails($fn, $ln, $un, $em, $pw) {
		...
		$defaultBanner = "assets/images/coverPhotos/default-cover-photo.jpg";

        $query = $this->con->prepare("INSERT INTO users (firstName, lastName, username, email, password, profilePic, banner)
                                        VALUES(:fn, :ln, :un, :em, :pw, :pic, :bn)");
		...
		$query->bindParam(":bn", $defaultBanner);
        
        return $query->execute();
    }
    
    private function validateFirstName($fn) {
        if(strlen($fn) > 1000 || strlen($fn) < 2) {
            array_push($this->errorArray, Constants::$firstNameCharacters);
        }
    }

    private function validateLastName($ln) {
        if(strlen($ln) > 1000 || strlen($ln) < 0) {
            array_push($this->errorArray, Constants::$lastNameCharacters);
        }
    }

    private function validateUsername($un) {
        if(strlen($un) > 1000 || strlen($un) < 5) {
